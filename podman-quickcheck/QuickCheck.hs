    ...
    =<< containerAttach
      ...
      ( defaultAttachQuery
          { _attachQuerylogs = Just True,
            ...
            _attachQuerystdout = Just True --, _attachQuerystdin = Just True})
          }
      )
      (go (10 :: Int))
  where
    ...
    spec =
      (mkSpecGenerator "registry.access.redhat.com/ubi8/ubi")
        { _specGeneratorname = Just demoName',
          ...
          -- _specGeneratorcommand = Just ["/bin/cat"]
          -- _specGeneratorcommand = Just ["/bin/bash", "-c", "while true; do date; sleep 5; done & echo holla; echo err > /proc/self/fd/2; echo catting; cat"]
          --          _specGeneratorentrypoint = Just ["/bin/cat"]
        }
