...
-- | todo: perhaps cleanup after the test ran?
ensureDelete :: FilePath -> IO ()
ensureDelete fp = do
  _ <- try . removeFile $ fp :: IO (Either IOException ())
  pure ()
...
main = do
  ...
  where
    ...
    cleanUp e = ensureDelete sktPath >> throwIO e
